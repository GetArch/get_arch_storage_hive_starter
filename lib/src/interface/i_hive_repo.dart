...
abstract class IHiveRepo<Ag extends IAgg<Id>, Id> with IRepo<Ag, Id> {}

///
/// 同步repo
/// 在进入App之前(依赖注入时)会初始化, 如果数据量过大(100MB+), 建议使用同步repo
class IHiveCrudRepo<Ag extends IAgg<Id>, Id> extends IHiveRepo<Ag, Id>
    with ICrudRepo<Ag, Id> {
  final HiveStorageSyncMx<Ag, Map<String, dynamic>, String> _storage;

  IHiveCrudRepo(this._storage);
...
/// 异步repo
/// 在首次查询的时候初始化数据
abstract class IHiveAsyncCrudRepo<Ag extends IAgg<Id>, Id>
    extends IHiveRepo<Ag, Id> with IAsyncCrudRepo<Ag, Id> {
  final HiveStorageASyncMx<Ag, Map<String, dynamic>, String> _storage;

  IHiveAsyncCrudRepo(this._storage);
